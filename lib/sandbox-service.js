...
function computeScore(scenario, prevScore, index, driverId) {
  switch (scenario) {
    case "random":
    ...
    case "all_well":
      return rand(80, 95);
    case "mixed":
      return MIXED_SCENARIO_SCORES[index] ?? rand(50, 95);
    case "high_risk":
      return rand(40, 65);
    case "improvement":
      return clamp(prevScore + rand(10, 15));
    case "degradation":
      if (DEGRADATION_DRIVERS.includes(driverId)) return clamp(prevScore - rand(15, 22));
...
export async function generateSandboxScores(type, scenario) {
  ...
  for (let i = 0; i < (drivers || []).length; i++) {
    ...
    const newScore = computeScore(effectiveScenario, prevScore, i, driver_id);
