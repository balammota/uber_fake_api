...
import {
  addMapistryRequestLog,
  checkMapistryRateLimit,
...
function logMapistryRequest(
  ...
  void addMapistryRequestLog({
    method: request.method,
    endpoint,
    status: response.status,
    apiKey: apiKey === "none" ? "none" : apiKey,
    duration: duration || Math.floor(Math.random() * 40) + 10,
  }).catch((err) => console.error("mapistry request log:", err?.message));
  return response;
}
...
export async function handleMapistryPublicRoute(request, endpoint, handler) {
  ...
  try {
    ...
    console.error("Mapistry public handler error:", err);
    ...
    return logMapistryRequest(request, endpoint, response, "none", Date.now() - start);
  }
  const response =
...
export async function handleMapistryProtectedRoute(request, endpoint, handler) {
  ...
  try {
    ...
    console.error("Mapistry handler error:", err);
    const response = mapistryJsonResponse(
      {
        error: "database_error",
        message: err?.message || "Database error",
