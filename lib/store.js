...
function seedData(store) {
  ...
  SEED_CAMPAIGNS.forEach((campaign) => {
    store.campaigns.set(campaign.campaign_id, { ...campaign });
  });
...
function getStore() {
  if (!globalThis.__uberFakeApiStore) {
    ...
    seedData(globalThis.__uberFakeApiStore);
  }
...
export function getAdsCampaignStats() {
  return getCampaignStats(getAllCampaigns());
}

export { getStoreSummaries, getStoreById, getStoreStats, isValidStoreId };
